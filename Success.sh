  ...
  "query": {
    "bool": {
      "must": [
        {
          "query_string": {
            "query": "type:\"nginx access_json\" AND request:forms AND request:submission AND request:receipt AND request:\"'${FORM}'\" AND NOT level:ERROR",
...
#curl -s -S -X POST -H "kbn-xsrf: kibana"  -d '{
#  "size": 500,
#  "sort": [
#    {
#      "@timestamp": {
