...
curl -s -S -X POST -u $VPNU:$VPNPASS -H "kbn-xsrf: kibana" -d '{"size":500,"query":{"bool":{"must":[{"query_string":{"analyze_wildcard":true,"query": "app:eeitt AND NOT app:\"eeitt-frontend\" AND \"registration of business\" AND NOT level:ERROR"}},{"range":{"@timestamp":{"gte":'${DATE_START_MILLIS}',"lte":'${DATE_END_MILLIS}',"format":"epoch_millis"}}}],"must_not":[]}}}' "${DATACENTRE}"
